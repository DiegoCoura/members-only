...
exports.getAllMessages = async () => {
  const { rows } = await pool.query(
    "SELECT messages.message_id, messages.user_id, messages.message, created_at, users.username FROM messages INNER JOIN users ON messages.user_id = users.user_id;"
  );
  return rows;
};
